...
uint8_t* pixl;
uint16_t w = 200; //width
uint16_t h = 100; //height
uint8_t c;
uint8_t cls;

int32_t alc_shm(uint32_t sz) {
	int8_t name[8];
	name[0] = '/';
	name[7] = 0;
...
void resz() {
	int32_t fd = alc_shm(w * h * 4); //each pixel 4 bytes 
...
void top_conf(void* data, struct xdg_toplevel* top, int32_t nw, int32_t nh, struct wl_array* states) {
	if (!nw && !nh) {
		return;
	}
...
void sh_ping(void* data, struct xdg_wm_base_shell* sh, uint32_t ser) {
